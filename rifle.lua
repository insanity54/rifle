local shoot_cooldown = rifle.init_cooldowns()

-- Can be overridden for custom behaviour
function rifle.can_use_gun(player, name)
   return true
end
...
function rifle.simple_register_gun(name, def)
   minetest.register_tool(rawf.also_register_loaded_tool(name, {
      ...
      on_use = function(itemstack, user)
         if not rifle.can_use_gun(user, name) then
            minetest.sound_play("click", { pos = user:get_pos() }, true)
            return
         end
         ...
         if result:get_name() == itemstack:get_name() then
            ...
            minetest.sound_play("reload", { pos = user:get_pos() }, true)
         end
      ...
      loaded_def.on_use = function(itemstack, user)
         ...
         local bullet_img = def.bullet_image or "bullet"
